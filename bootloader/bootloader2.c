...
uint32 getAddress(uint8 *a2) {
	return ((uint32)a2[2] << 0x10) | ((uint32)a2[1] << 0x08) | (uint32)a2[0];
}
...
void call_user_start() {
	...
	if ((romconf.byte1 & BYTE1_xxxMODE == 0) || (romconf.byte1 & BYTE1_TESTMODE)) {
	...
	if (loadImage(runAddr) == FAIL) {
		if ((romconf.byte1 & BYTE1_xxxMODE == 0) && (romconf.byte1 & BYTE1_TESTMODE)) return;
